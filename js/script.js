...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function calculateDailyVrchatDay(date) {
        ...
        if (normalizedDate.getTime() < targetDate.getTime())
            return null;
    ...
    let introElement, 
    /* meaningElement removed */ principlesListElement, principlesNoteElement, calendarGrid, monthYearDisplay, weekdaysContainer, dailyInfoElement, dailyDescElement, dailyWarnElement, dailyRulesElement, countdownContainer, countdownTimerDisplay, countdownDayLabel, closingCountdownContainer, closingCountdownTimerDisplay, closingCountdownDayLabel, happeningNowContainer, happeningNowMessage, happeningNowLink, verifiedInfoElement, certifiedInfoElement, verifiedListElement, certifiedListElement, rosefishInfoElement, rosefishVoteInfoElement, rosefishGuidelinesElement, rosefishMembersListElement, luxuryMottoElement, luxuryProductsListElement, prevMonthButton, nextMonthButton, gotoTodayButton, startSunButton, startMonButton, knownInfoElement, knownListElement, registeredInfoElement, registeredListElement, fishStatusListElement;
    ...
    function loadIntro(data) {
        if (!introElement || !data?.info?.intro)
            return;
    ...
    function loadFishGroups(data) {
        ...
        if (!Array.isArray(communities)) {
            ...
            if (verifiedListElement)
                verifiedListElement.innerHTML = '<li class="error-message"><i>Could not load communities data (invalid structure).</i></li>';
            if (certifiedListElement)
                certifiedListElement.innerHTML = '<li class="error-message"><i>Could not load communities data (invalid structure).</i></li>';
            if (registeredListElement)
                registeredListElement.innerHTML = '<li class="error-message"><i>Could not load communities data (invalid structure).</i></li>';
            if (knownListElement)
                knownListElement.innerHTML = '<li class="error-message"><i>Could not load communities data (invalid structure).</i></li>';
            if (fishStatusListElement)
                fishStatusListElement.innerHTML = '<li class="error-message"><i>Could not load communities data (invalid structure).</i></li>';
            return;
        }
        ...
        if (registeredListElement)
            registeredListElement.innerHTML = '';
        if (knownListElement)
            knownListElement.innerHTML = '';
        if (fishStatusListElement)
            fishStatusListElement.innerHTML = ''; // Clear FISH status list
        let verifiedCount = 0;
        ...
        communities.forEach((community) => {
            ...
            if (community.owner)
                textContentHTML += `<span>${escapeHtml(community.owner)}</span>`; // Removed "Owner:" label
                    ...
                    if (typeof link === 'string' && link.trim() !== '') {
                        ...
                        try {
                            ...
                            if (hostname.includes('discord.gg')) {
                                iconClass = 'fab fa-discord fa-fw';
                                iconTitle = 'Discord';
                            }
                            else if (hostname.includes('twitter.com')) {
                                iconClass = 'fab fa-x-twitter fa-fw';
                                iconTitle = 'Twitter/X';
                            }
                            else if (hostname.includes('patreon.com')) {
                                iconClass = 'fab fa-patreon fa-fw';
                                iconTitle = 'Patreon';
                            }
                            else if (hostname.includes('ko-fi.com')) {
                                iconClass = 'fas fa-mug-saucer fa-fw';
                                iconTitle = 'Ko-fi';
                            }
                            else if (hostname.includes('youtube.com') || hostname.includes('youtu.be')) {
                                iconClass = 'fab fa-youtube fa-fw';
                                iconTitle = 'YouTube';
                            }
                            else if (hostname.includes('booth.pm')) {
                                iconClass = 'fas fa-store fa-fw';
                                iconTitle = 'Booth';
                            }
                            else if (hostname.includes('github.com')) {
                                iconClass = 'fab fa-github fa-fw';
                                iconTitle = 'GitHub';
                            }
                            else if (hostname.includes('twitch.tv')) {
                                iconClass = 'fab fa-twitch fa-fw';
                                iconTitle = 'Twitch';
                            }
                            else if (hostname.includes('github.io') || url.pathname.length > 1) {
        ...
        if (data?.daily_vrchat && Array.isArray(data.daily_vrchat.special_days)) {
            data.daily_vrchat.special_days.forEach((item) => {
                if (item && typeof item.day === 'number' && Number.isInteger(item.day) && item.day >= 0 && typeof item.name === 'string' && item.name.trim() !== '') {
                    ...
                    console.warn("Skipping invalid daily-number-based special day entry:", item);
                }
            });
        }
    ...
    function renderCalendar(month, year) {
        if (!calendarGrid || !monthYearDisplay || !weekdaysContainer) {
            ...
            if (calendarGrid)
                calendarGrid.innerHTML = "<p class='error-message' style='grid-column: 1 / -1; text-align: center;'><i>Error: Calendar elements missing.</i></p>";
            return;
        }
    ...
    function createDayCell(dayNumber, isOtherMonth, cellDate = null, isToday = false) {
        ...
        if (isOtherMonth) {
            ...
            if (dailyDay !== null && dailySpecialDaysMap.has(dailyDay)) {
                ...
                if (dailySpecialDay.css_class)
                    dayCell.classList.add(dailySpecialDay.css_class);
                ...
                if (linkTitle)
                    wrapperElement.title = linkTitle;
    ...
    function updateWheelSpinTimer() {
        ...
        if (!spinCountdown || !spinTimer || !window.wheelSpinConfig)
            return;
        const now = new Date();
        const nowUTC = now.getTime();
        const cfg = window.wheelSpinConfig;
        const target = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), cfg.hour, cfg.minute, 0, 0));
    ...
    function updateStartDaySelection(newStartDay) {
        if (startDayOfWeekSetting === newStartDay || !startSunButton || !startMonButton)
            return;
    ...
    function initializePage() {
        ...
        Promise.resolve()
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            const errorMsg = `<i class="error-message">Error loading data: ${escapeHtml(errorMessage)}</i>`;
            const errorLi = `<li class="error-message"><i>Error loading data: ${escapeHtml(errorMessage)}</i></li>`;
            let isPrimaryError = error instanceof Error && error.message.includes("Primary data");
            let isCommunityError = error instanceof Error && error.message.includes("Community data");
            ...
            if (isPrimaryError || !isCommunityError) { // Show primary errors if primary failed OR if it's a general error
                if (introElement)
                    introElement.innerHTML = errorMsg;
                if (principlesListElement)
                    principlesListElement.innerHTML = errorLi;
                if (principlesNoteElement)
                    principlesNoteElement.textContent = '';
                if (dailyInfoElement)
                    dailyInfoElement.innerHTML = errorMsg;
                if (dailyDescElement)
                    dailyDescElement.innerHTML = errorMsg;
                if (dailyRulesElement)
                    dailyRulesElement.innerHTML = errorMsg;
                if (dailyWarnElement)
                    dailyWarnElement.innerHTML = errorMsg;
                if (calendarGrid)
                    calendarGrid.innerHTML = `<div class='error-message' style='grid-column: 1 / -1; text-align: center; padding: 20px;'>${errorMsg}</div>`;
                if (monthYearDisplay)
                    monthYearDisplay.textContent = "Error";
                if (rosefishInfoElement)
                    rosefishInfoElement.innerHTML = errorMsg;
                if (rosefishVoteInfoElement)
                    rosefishVoteInfoElement.innerHTML = errorMsg;
                if (rosefishGuidelinesElement)
                    rosefishGuidelinesElement.innerHTML = errorMsg;
                if (rosefishMembersListElement)
                    rosefishMembersListElement.innerHTML = errorLi;
                if (luxuryMottoElement)
                    luxuryMottoElement.innerHTML = errorMsg;
                if (luxuryProductsListElement)
                    luxuryProductsListElement.innerHTML = errorLi;
            ...
            if (isCommunityError || !isPrimaryError) { // Show community errors if community failed OR if it's a general error
                if (verifiedInfoElement)
                    verifiedInfoElement.innerHTML = errorMsg;
                ...
                if (verifiedListElement)
                    verifiedListElement.innerHTML = errorLi;
                if (certifiedListElement)
                    certifiedListElement.innerHTML = errorLi;
                if (registeredListElement)
                    registeredListElement.innerHTML = errorLi; // Add registered list error
                if (knownListElement)
                    knownListElement.innerHTML = errorLi; // Add known list error
...
(function () {
    ...
    if (page) {
        ...
        if (page === 'daily') {
            ...
            if (page === 'home')
                bgUrl = 'images/HOME_BG.webp';
            else if (page === 'rosefish')
                bgUrl = 'images/ROSE_FISH_BG.webp';
            else if (page === 'fishcraft')
                bgUrl = 'images/FISH_CRAFT_BG.webp';
            else if (page === 'support')
                bgUrl = 'images/SUPPORT_BG.webp';
            else if (page === 'ratfishrace')
                bgUrl = 'images/RAT_FISH_RACE_BG.webp';
            else if (page === 'communities')
                bgUrl = 'images/COMMUNITIES_BG.webp';
            else if (page === 'aquarium')
                bgUrl = 'images/AQUARIUM_BG.webp';
