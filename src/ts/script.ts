...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const escapeHtml = (unsafe: unknown): string => {
        if (!unsafe || typeof unsafe !== 'string') return unsafe === 0 ? '0' : '';
        return unsafe
             .replace(/&/g, "&amp;")
             .replace(/</g, "&lt;")
             .replace(/>/g, "&gt;")
    ...
    function getOrdinalSuffix(num: unknown): string {
    ...
    const getFaviconUrl = (resource: string): string => {
        try {
    ...
    const setLinkIcon = (a: HTMLAnchorElement, link: string): void => {
        ...
        img.loading = 'lazy';
        img.src = url;
        img.onerror = function(this: HTMLImageElement) { this.outerHTML = '<i class="fas fa-link"></i>'; };
        a.innerHTML = '';
        a.appendChild(img);
    };
   ...
   function calculateDailyVrchatDay(date: Date): number | null {
        const normalizedDate = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate()));
        if (normalizedDate.getTime() < targetDate.getTime()) return null;
        const dayDifference = Math.floor((normalizedDate.getTime() - targetDate.getTime()) / oneDay);
        return (dayDifference >= 0) ? dayDifference : null;
   }

   // --- Image Mapping ---
    const IMAGE_MAP: Record<string, string | undefined> = {
    ...
    function generateCommunityImageKey(name: string): string | null {
        if (!name) return null;
    ...
    let currentMonth: number = today.getUTCMonth();
    let currentYear: number = today.getUTCFullYear();
    let startDayOfWeekSetting: number = 1;
    ...
    const WEEKDAY_WORLD_IDS: Record<number, string> = {
    ...
    const WEEKDAY_INSTANCE_IDS: Record<number, string | null> = {
    ...
    const WORLD_IDS: Record<number, string> = WEEKDAY_WORLD_IDS;

    const VRC_GROUP_ID: string = "grp_2b910dc4-e984-4fd5-813c-877edcea29d2";
    ...
    let specialDaysMap = new Map<number, unknown>();
    ...
    let dailySpecialDaysMap = new Map<number, SpecialDayInfo>();
    ...
    let introElement: HTMLElement | null,
        /* meaningElement removed */ principlesListElement: HTMLElement | null,
        principlesNoteElement: HTMLElement | null,
        calendarGrid: HTMLElement | null,
        monthYearDisplay: HTMLElement | null,
        weekdaysContainer: HTMLElement | null,
        dailyInfoElement: HTMLElement | null,
        dailyDescElement: HTMLElement | null,
        dailyWarnElement: HTMLElement | null,
        dailyRulesElement: HTMLElement | null,
        countdownContainer: HTMLElement | null,
        countdownTimerDisplay: HTMLElement | null,
        countdownDayLabel: HTMLElement | null,
        closingCountdownContainer: HTMLElement | null,
        closingCountdownTimerDisplay: HTMLElement | null,
        closingCountdownDayLabel: HTMLElement | null,
        happeningNowContainer: HTMLElement | null,
        happeningNowMessage: HTMLElement | null,
        happeningNowLink: HTMLElement | null,
        verifiedInfoElement: HTMLElement | null,
        certifiedInfoElement: HTMLElement | null,
        verifiedListElement: HTMLElement | null,
        certifiedListElement: HTMLElement | null,
        rosefishInfoElement: HTMLElement | null,
        rosefishVoteInfoElement: HTMLElement | null,
        rosefishGuidelinesElement: HTMLElement | null,
        rosefishMembersListElement: HTMLElement | null,
        luxuryMottoElement: HTMLElement | null,
        luxuryProductsListElement: HTMLElement | null,
        prevMonthButton: HTMLElement | null,
        nextMonthButton: HTMLElement | null,
        gotoTodayButton: HTMLElement | null,
        startSunButton: HTMLElement | null,
        startMonButton: HTMLElement | null,
        knownInfoElement: HTMLElement | null,
        knownListElement: HTMLElement | null,
        registeredInfoElement: HTMLElement | null,
        registeredListElement: HTMLElement | null,
        fishStatusListElement: HTMLElement | null;

    // Function to get references to all elements after DOM is ready
    function getAllElements(): void {
    ...
    function loadIntro(data: { info?: { intro?: string } } | undefined): void {
        if (!introElement || !data?.info?.intro) return;
        introElement.textContent = data.info.intro;
    }

    function loadMeaningAndPrinciples(data: { info?: { community_principles?: { principles?: Array<{ title?: string; description?: string }>; note?: string } } } | undefined): void {
        // Principles List
        if (principlesListElement && data?.info?.community_principles?.principles) {
            principlesListElement.innerHTML = '';
            data.info.community_principles.principles.forEach((principle: { title?: string; description?: string }) => {
    ...
    function loadDailyInfo(data: { daily_vrchat?: { daily_vrchat_info?: { extra_info?: string; description?: string; rules?: string; warning?: string } } } | undefined): void {
    ...
    function loadCommunityInfo(data: { fish_communities?: { verified_info?: string; certified_info?: string; known_info?: string } } | undefined): void {
   ...
   function loadFishGroups(data: { community_groups?: Array<{
        group_name?: string;
        owner?: string;
        tags?: string[];
        icon_url?: string;
        ...
        communities.forEach((community: {
            ...
            if (isVerifiedOrCertified && vrchatLink && (vrchatLink.startsWith('http') || vrchatLink.startsWith('vrchat://'))) {
                ...
                if (vrchatLink.startsWith('http')) {
                    ...
                    (a.querySelector('.community-link-icon-img') as HTMLImageElement).onerror = function(this: HTMLImageElement) { this.outerHTML = '<i class="fas fa-link"></i>'; };
                } else {
            ...
            if (community.tags && Array.isArray(community.tags)) {
                ...
                } else if (community.tags.includes('FISH') && fishStatusListElement) {
    ...
    function loadRoseFishInfo(data: { rose_fish?: { rose_fish_info?: string; vote_info?: string; avatar_guidelines?: string } } | undefined): void {
        ...
        if (rosefishGuidelinesElement && data?.rose_fish?.avatar_guidelines) {
            let guidelinesHtml: string = escapeHtml(data.rose_fish.avatar_guidelines)
                                  .replace(/\n/g, '<br>')
                                  .replace(/\*\*(.*?)\*\*/g, '<b>$1</b>');
    ...
    function loadRoseFishMembers(): void {
    ...
    function handleResponseJson(response: Response): Promise<unknown> {
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        return response.text().then((text: string) => JSON.parse(text));
    }

    function parseMemberLines(nameLine: string, infoLine: string): [string | null, string | null] {
    ...
    function createMemberListItem(name: string, info: string): HTMLLIElement {
        const li = document.createElement('li');
    ...
    function handleMemberError(error: unknown): void {
    ...
    function loadProducts(data: { store?: { products?: Array<{ item: string; link: string; price?: string }> } } | undefined): void {
        ...
        if (productsContainer) {
            ...
            data.store.products.forEach((product: { item: string; link: string; price?: string }) => {
    ...
    function processSpecialDays(data: { daily_vrchat?: {
        special_days?: Array<{ day: number; name: string; description?: string; link?: string; css_class?: string }>;
        winner?: Array<{ day: number; name: string; description?: string; link?: string; css_class?: string; gif_url?: string; spun_at?: string }>;
        "winner "?: Array<{ day: number; name: string; description?: string; link?: string; css_class?: string; gif_url?: string; spun_at?: string }>;
        wheel_spin_day?: number;
        ...
        if (data?.daily_vrchat && Array.isArray(data.daily_vrchat.special_days)) {
            data.daily_vrchat.special_days.forEach((item: { day: number; name: string; description?: string; link?: string; css_class?: string }) => {
        ...
        const winnerArray = data?.daily_vrchat?.["winner"] || data?.daily_vrchat?.["winner "];
        if (Array.isArray(winnerArray)) {
            winnerArray.forEach((item: { day: number; name: string; description?: string; link?: string; css_class?: string; gif_url?: string; spun_at?: string }) => {
                if (item && typeof item.day === 'number' && Number.isInteger(item.day) && item.day >= 0 && typeof item.name === 'string' && item.name.trim() !== '') {
                    if (!dailySpecialDaysMap.has(item.day)) {
                        dailySpecialDaysMap.set(item.day, {
                            name: item.name,
                            description: item.description,
                            link: item.link,
                            css_class: item.css_class || 'random-day'
                        });
                    }
                } else { console.warn("Skipping invalid winner entry:", item); }
            });

            // Find latest winner by spun_at and update the spin GIF
            const spinSection = document.getElementById('daily-spin-section');
            if (spinSection) {
                const spinImg = spinSection.querySelector('.daily-spin-gif') as HTMLImageElement | null;
                if (spinImg) {
                    let latestGifUrl: string | null = null;
                    let latestDay: number = 0;
                    let latestDate: Date | null = null;
                    winnerArray.forEach((item: { day: number; gif_url?: string; spun_at?: string }) => {
                        if (item && item.gif_url && item.spun_at) {
                            const d = new Date(item.spun_at);
                            if (!isNaN(d.getTime()) && (latestDate === null || d > latestDate)) {
                                latestDate = d;
                                latestGifUrl = item.gif_url;
                                latestDay = item.day;
                            }
                        }
                    });
                    if (latestGifUrl) {
                        spinImg.src = latestGifUrl;
                        spinImg.alt = "Wheel Spin for Day " + latestDay;
                    }
                }
            }
        }
    ...
    function renderCalendar(month: number, year: number): void {
       ...
       if (dailySpecialDaysMap.size === 0) {
        ...
        const weekdayNamesShort: string[] = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];

        // Always show all weekdays, respecting the start day setting
        let orderedWeekdays: string[] = (startDayOfWeekSetting === 1)
            ? weekdayNamesShort
            : [...weekdayNamesShort.slice(6), ...weekdayNamesShort.slice(0, 6)];

        orderedWeekdays.forEach((dayName: string) => {
        ...
        try {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                const isToday: boolean = (year === todayUTC.getUTCFullYear() && month === todayUTC.getUTCMonth() && day === todayUTC.getUTCDate());
            ...
            calendarGrid!.innerHTML = `<p class='error-message' style='grid-column: 1 / -1; text-align: center;'><i>Error generating calendar days: ${escapeHtml(errorMessage)}</i></p>`;
        }
    }

    function createDayCell(dayNumber: number, isOtherMonth: boolean, cellDate: Date | null = null, isToday: boolean = false): HTMLDivElement {
        ...
        dayNumberSpan.textContent = String(dayNumber);
        ...
        if (isOtherMonth) {
            ...
            if (dailyDay !== null && dailySpecialDaysMap.has(dailyDay)) {
                ...
                const wrapperElement: HTMLElement = document.createElement(linkHref ? 'a' : 'div');
                if (linkHref) {
                    (wrapperElement as HTMLAnchorElement).href = linkHref;
                    (wrapperElement as HTMLAnchorElement).target = "_blank";
                    (wrapperElement as HTMLAnchorElement).rel = "noopener noreferrer";
                }
        ...
        try {
            ...
            if (nowUTCMillis >= todayStartUTC.getTime() && nowUTCMillis < todayCloseUTC.getTime()) {
                ...
                if (instanceId !== null) {
                     ...
                     (happeningNowLink as HTMLAnchorElement).href = vrchatLink;
    ...
    function updateWheelSpinTimer(): void {
        const spinCountdown = document.getElementById('spin-countdown-container');
        const spinTimer = document.getElementById('spin-countdown-timer');
        if (!spinCountdown || !spinTimer || !window.wheelSpinConfig) return;

        const now = new Date();
        const nowUTC = now.getTime();
        const cfg = window.wheelSpinConfig;

        const target = new Date(Date.UTC(
            now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(),
            cfg.hour, cfg.minute, 0, 0
        ));
        let daysUntil = (cfg.day - now.getUTCDay() + 7) % 7;
        target.setUTCDate(target.getUTCDate() + daysUntil);

        if (nowUTC >= target.getTime()) {
            target.setUTCDate(target.getUTCDate() + 7);
        }

        const diff = target.getTime() - nowUTC;
        spinTimer.textContent = diff <= 0 ? "Spinning now!" : formatTimeDifference(diff);
        spinCountdown.style.display = 'block';
    }

    function formatTimeDifference(differenceMillis: number): string {
    ...
    function displayTimerErrorState(message: string): void {
    ...
    function updateStartDaySelection(newStartDay: number): void {
        ...
        Promise.resolve()
            ...
            .then((primaryData: unknown) => {
                const pd = primaryData as {
                    ...
                    store?: { products?: Array<{ item: string; link: string; price?: string }> };
                };
                processSpecialDays(pd);
                loadProducts(pd);

                // Extract wheel spin schedule from gist data
                const dv = pd?.daily_vrchat;
                if (dv && typeof dv.wheel_spin_day === 'number') {
                    // Convert Python weekday (Monday=0) to JS weekday (Sunday=0)
                    window.wheelSpinConfig = {
                        day: (dv.wheel_spin_day + 1) % 7,
                        hour: dv.wheel_spin_hour ?? 22,
                        minute: dv.wheel_spin_minute ?? 50
                    };
                }
                ...
                updateEventTimers();
                updateWheelSpinTimer();
                if (!window.timerElementsMissing) {
                   setInterval(updateEventTimers, 1000);
                   setInterval(updateWheelSpinTimer, 1000);
                }

                document.addEventListener('click', function(e: MouseEvent) {
                    const target = e.target;
                    if (target instanceof HTMLImageElement && target.src && target.src.includes('daily_vrchat')) {
                        e.preventDefault();
                        e.stopPropagation();
                        const now = new Date();
                        ...
                        if (dailyDay !== null) {
                            let vrchatLink: string | null = null;
                ...
                function attachDailyImageHandlers(): void {
                    const dailyImages = document.querySelectorAll('img[src="images/daily_vrchat.webp"]');
                    dailyImages.forEach((img: Element) => {
                        (img as HTMLImageElement).style.cursor = 'pointer';
                        (img as HTMLImageElement).setAttribute('title', 'Click to join today\'s Daily VRChat event');
                    });
                }
            ...
            .catch((error: unknown) => {
                ...
                const errorMessage: string = error instanceof Error ? error.message : String(error);
                const errorMsg: string = `<i class="error-message">Error loading data: ${escapeHtml(errorMessage)}</i>`;
                const errorLi: string = `<li class="error-message"><i>Error loading data: ${escapeHtml(errorMessage)}</i></li>`;
                let isPrimaryError: boolean = error instanceof Error && error.message.includes("Primary data");
                let isCommunityError: boolean = error instanceof Error && error.message.includes("Community data");
                ...
                if (isPrimaryError || !isCommunityError) { // Show primary errors if primary failed OR if it's a general error
                    ...
                    displayTimerErrorState(`Primary data fetch failed: ${errorMessage}`);
                }
                 if (isCommunityError || !isPrimaryError) { // Show community errors if community failed OR if it's a general error
                    ...
                    fetch('data/communities.json')
                        ...
                        .then((data: unknown) => {
                            loadFishGroups(data as { community_groups?: Array<{
                                group_name?: string;
                                owner?: string;
                                tags?: string[];
                                icon_url?: string;
...
const PAGE_BG_IMAGES: Record<string, string> = {
    home: 'images/DVRC_BG.webp',
    daily: 'images/DVRC_BG.webp',
    rosefish: 'images/ROSE_FISH_BG.webp',
    fishcraft: 'images/FISH_CRAFT_BG.webp',
    support: 'images/SUPPORT_BG.webp',
    ratfishrace: 'images/RAT_FISH_RACE_BG.webp',
    communities: 'images/COMMUNITIES_BG.webp',
    aquarium: 'images/AQUARIUM_BG.webp'
};

// Preload all background images for caching
function preloadBackgroundImages(): void {
    Object.values(PAGE_BG_IMAGES).forEach((src: string) => {
        const img = new Image();
        img.src = src;
...
function lazyLoadBackground(page: string, bgDiv: HTMLElement): void {
...
(function() {
    ...
    const page: string = pageMap[path] || '';
    if (page) {
        ...
        if (page === 'daily') {
            ...
            const bgImages: string[] = [
                'images/DVRC_BG.webp',
                'images/DVRC_BG_1.webp',
                'images/DVRC_BG_2.webp',
                'images/DVRC_BG_3.webp',
                'images/DVRC_BG_4.webp',
                'images/DVRC_BG_5.webp'
            ];
            let idx = 0;
            let isFirst = true;
            function setBg(fade: boolean = true): void {
                if (bgDiv) {
                    if (fade && !isFirst) {
            ...
            if (page === 'home') bgUrl = 'images/HOME_BG.webp';
            else if (page === 'rosefish') bgUrl = 'images/ROSE_FISH_BG.webp';
            else if (page === 'fishcraft') bgUrl = 'images/FISH_CRAFT_BG.webp';
            else if (page === 'support') bgUrl = 'images/SUPPORT_BG.webp';
            else if (page === 'ratfishrace') bgUrl = 'images/RAT_FISH_RACE_BG.webp';
            else if (page === 'communities') bgUrl = 'images/COMMUNITIES_BG.webp';
            else if (page === 'aquarium') bgUrl = 'images/AQUARIUM_BG.webp';
